...
print("\nFile 2 ("+file2_name+") has "+str(file2_solns)+" solutions of "+str(len(df2[file2_quat_scalar_fieldname]))+" total ("+str((file2_solns/len(df2[file1_quat_scalar_fieldname]))*100)[:5]+"%)")
...
for name in file1_soln_names:
    for other_name in file2_soln_names:
            if name == other_name:
                common_names+=[name]
